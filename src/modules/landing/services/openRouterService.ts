...
import { getOpenRouterKey } from "./apiKeyService";
import { FALLBACK_OPENROUTER_MODELS } from "../constants";
...
export const getAvailableModels = async (providedApiKey?: string): Promise<OpenRouterModel[]> => {
    const apiKey = providedApiKey || getOpenRouterKey();
    if (!apiKey) return FALLBACK_OPENROUTER_MODELS;
...
const callOpenRouter = async (messages: any[], modelId: string, jsonMode: boolean = false) => {
...
export const generateTextContent = async (formData: FormData): Promise<any> => {
    ...
    const modelId = formData.openRouterModelId || 'anthropic/claude-3.5-sonnet';

    const blocks = isLandingPage ? formData.landingBlocks : formData.leadMagnetBlocks;
    const blockList = blocks.join(', ');

    const jsonHint = isLandingPage
        ? `{"hero": {"headline": "...", ...}, "painPoints": ["...", ...], ...}`
        : `{"title": {"mainTitle": "...", ...}, "chapters": [{"title": "...", "content": "..."}], ...}`;
    ...
    try {
        ...
        throw new Error(error.message || "Не удалось сгенерировать текст через OpenRouter");
    }
};
...
export const generateHtmlLayout = async (textContent: any, formData: FormData): Promise<string> => {
    ...
    try {
        ...
        throw new Error(error.message || "Не удалось сгенерировать HTML через OpenRouter");
    }
};
...
export const refineHtml = async (currentHtml: string, userRequest: string, formData: FormData): Promise<string> => {
    ...
    try {
        ...
        throw new Error(error.message || "Не удалось обновить HTML через OpenRouter");
    }
};
